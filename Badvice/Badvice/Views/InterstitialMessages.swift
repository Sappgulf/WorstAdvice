...
struct SettingsTabView: View {
    ...
    private static let isUITestingLaunch = ProcessInfo.processInfo.arguments.contains("-ui-testing")
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: Theme.sectionSpacing + 8) {
                    // Header Area
                    VStack(spacing: 8) {
                        Button {
                            gearIsSpinning = true
                    ...
                    VStack(spacing: Theme.sectionSpacing + 4) {
    ...
    private var generationEngineOptionsCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                Label("On-Device Generation Options", systemImage: "internaldrive")
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(primaryText)
                Spacer()
                Text("Choose")
                    ...
                    .padding(.horizontal, 8)
                    .padding(.vertical, 4)
                    .background(Capsule(style: .continuous).fill(secondaryText.opacity(0.10)))
            }

            VStack(spacing: 8) {
                ForEach(AdviceGenerationProvider.allCases) { provider in
                    generationEngineOptionRow(provider)
                }
            }
        }
        .padding(12)
        ...
        .accessibilityIdentifier("settings.generationEngine.catalog")
    }

    private func generationEngineOptionRow(_ provider: AdviceGenerationProvider) -> some View {
        let isSelected = viewModel.preferredGenerationProvider == provider
        let isAppleProvider = provider == .appleOnDevice
        let statusLabel = generationEngineStatusLabel(for: provider)
        let statusTint = generationEngineStatusTint(for: provider)

        return Button {
            guard viewModel.preferredGenerationProvider != provider else { return }
            HapticsManager.playSelection(isEnabled: viewModel.hapticsEnabled)
            viewModel.preferredGenerationProvider = provider
        } label: {
            HStack(alignment: .center, spacing: 12) {
                Image(systemName: generationEngineSymbolName(for: provider))
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(generationEngineSubtitle(for: provider))
                        .font(.caption)
                        .foregroundStyle(secondaryText)
                        .fixedSize(horizontal: false, vertical: true)
                }
                ...
                VStack(alignment: .trailing, spacing: 6) {
                    ...
                    Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(isSelected ? accent : secondaryText.opacity(0.45))
                }
            }
            .frame(maxWidth: .infinity, alignment: .leading)
            ...
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(isSelected ? accent.opacity(0.10) : cardColor.opacity(0.35))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(
                        isSelected ? accent.opacity(0.25) : secondaryText.opacity(0.10),
                        lineWidth: 1
                    )
            )
        }
        .buttonStyle(.plain)
        .accessibilityIdentifier("settings.generationEngine.option.\(provider.rawValue)")
        .accessibilityLabel(provider.title)
        .accessibilityValue(isSelected ? "Selected" : statusLabel)
        .accessibilityHint(
            isAppleProvider
                ? "Choose Apple on-device generation. Install status appears below."
                : "Choose this generation engine."
        )
    }

    private func generationEngineSymbolName(for provider: AdviceGenerationProvider) -> String {
        switch provider {
        case .auto:
    ...
    private func generationEngineSubtitle(for provider: AdviceGenerationProvider) -> String {
        switch provider {
        case .auto:
            return "Uses Apple local model when ready, otherwise falls back to Classic."
        case .classic:
            return "Built-in generator. Always available and instant."
        case .appleOnDevice:
            return "Apple Intelligence local model. Private on-device generation."
        }
    }

    private func generationEngineStatusLabel(for provider: AdviceGenerationProvider) -> String {
        switch provider {
        case .auto:
            return viewModel.appleOnDeviceModelStatusKey == "ready" ? "Smart Ready" : "Fallback Ready"
        case .classic:
            return "Installed"
        case .appleOnDevice:
    ...
    private func generationEngineStatusTint(for provider: AdviceGenerationProvider) -> Color {
        switch provider {
        case .classic:
            return secondaryText
        case .auto:
            return viewModel.appleOnDeviceModelStatusKey == "ready" ? accent : .orange
        case .appleOnDevice:
            return appleModelStatusTint
        }
    }
    ...
    private var appleOnDeviceModelStatusCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            ViewThatFits(in: .horizontal) {
                appleOnDeviceModelActionButtons(horizontal: true)
                appleOnDeviceModelActionButtons(horizontal: false)
            }
        }
        .padding(12)
        .background(
    ...
    @ViewBuilder
    private func appleOnDeviceModelActionButtons(horizontal: Bool) -> some View {
