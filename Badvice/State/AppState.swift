...
final class AppSettingsEntity {
    ...
    private static func sanitizedTabOrder(_ candidate: [AppTab]) -> [AppTab] {
        var ordered: [AppTab] = []
        var seen = Set<AppTab>()
        for tab in candidate where AppTab.enabledTabs.contains(tab) && seen.insert(tab).inserted {
            ordered.append(tab)
        }
...
final class SettingsViewModel {
    ...
    private func applyPrimaryTabOrder(_ primaryItems: [AppTab]) {
        ...
        let overflowItems = AppTab.allCases.filter { $0.isEnabled && !pinnedTabs.contains($0) }
...
enum SocialBackendFactory {
    static func make() -> any SocialBackend {
        ...
        let isRunningUnderXCTest = ProcessInfo.processInfo.isRunningUnderXCTest
